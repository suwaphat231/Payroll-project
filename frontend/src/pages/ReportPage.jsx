    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-white">รายงานบุคลากร • Payroll • การลา</h1>
            <p className="text-sm text-slate-400">มุมมองสำหรับผู้ดูแลระบบ (Admin/HR/Accounting)</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <input type="date" value={filters.start} onChange={e=>setFilters(f=>({...f,start:e.target.value}))} className="rounded-xl border border-slate-600 bg-slate-700 text-white px-3 py-2 text-sm"/>
            <input type="date" value={filters.end} onChange={e=>setFilters(f=>({...f,end:e.target.value}))} className="rounded-xl border border-slate-600 bg-slate-700 text-white px-3 py-2 text-sm"/>
            <select value={filters.dept} onChange={e=>setFilters(f=>({...f,dept:e.target.value}))} className="rounded-xl border border-slate-600 bg-slate-700 text-white px-3 py-2 text-sm">
              {depts.map(d=> <option key={d}>{d}</option>)}
            </select>
            <input value={filters.search} onChange={e=>setFilters(f=>({...f,search:e.target.value}))} placeholder="ค้นหาพนักงาน: ชื่อ/รหัส" className="rounded-xl border border-slate-600 bg-slate-700 text-white placeholder-slate-400 px-3 py-2 text-sm"/>
            <button onClick={exportCSV} className="rounded-xl bg-slate-700 px-4 py-2 text-sm font-medium text-white hover:bg-slate-600 shadow-lg">Export CSV</button>
            <button onClick={printPDF} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 shadow-lg">พิมพ์ / PDF</button>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card title="Top OT (ตามมูลค่า OT ในช่วงที่เลือก)">
            <table className="w-full text-sm">
              ...
              <tbody>
                {topOT.map(r => (
                  <tr key={r.code} className="odd:bg-slate-800/30 even:bg-slate-700/50">
                    <Td>{r.code}</Td>
                    <Td>{r.name}</Td>
                    <Td>{r.dept}</Td>
          ...
          <Card title="วันลาตามประเภท">
            <table className="w-full text-sm">
              ...
              <tbody>
                {leaveByType.map((r,i) => (
                  <tr key={i} className="odd:bg-slate-800/30 even:bg-slate-700/50">
                    <Td>{r.name}</Td>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {filteredEmployees.map(e => (
                  <tr key={e.code} className="odd:bg-slate-800/30 even:bg-slate-700/50">
                    <Td>{e.code}</Td>
                    <Td>{e.name}</Td>
                    <Td>{e.dept}</Td>
                    <Td>{e.status}</Td>
...
function KPI({ label, value, subtitle }) {
  return (
    <div className="rounded-2xl bg-slate-800/50 p-4 shadow-lg ring-1 ring-slate-700">
...
function Card({ title, className = "", children }) {
  return (
    <div className={`rounded-2xl bg-slate-800/50 p-4 shadow-lg ring-1 ring-slate-700 ${className}`}>
      <div className="mb-2 text-sm font-semibold uppercase tracking-wider text-slate-300">{title}</div>
      {children}
    </div>
  );
}
function Th({ children, className = "" }) {
  return <th className={`px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-slate-300 ${className}`}>{children}</th>;
}
